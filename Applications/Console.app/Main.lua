...
local workspace, window, menu = system.addWindow(GUI.filledWindow(1, 1, 82, 28, colors.black))
...
disp.draw = function(disp)
	...
	for i = lineFrom, #lines do
		screen.drawText(x, y, colors.white, lines[i])
		y = y + 1
	end

	local text = "> " .. input
	screen.drawText(x, y, colors.white, text)
	screen.drawText(x + unicode.len(text), y, colors.lightBlue, "┃")
end
